...
function revrot(str, sz) {
    ...
    for (let i = 0, len = output.length - (output.length % sz); i < len; i += sz) {
        var sumOfCubesDigits = 0, tmpArr = []
        for (let j = i; j < i+sz; j++) {
            sumOfCubesDigits += output[j]**3;
            tmpArr.push(output[j]);
        }
        (sumOfCubesDigits % 2 === 0) ? tmpArr.reverse() : tmpArr.push(tmpArr.shift());
        result.push(...tmpArr)   
    }
    return result.join('');
}
